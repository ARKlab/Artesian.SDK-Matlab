...
function [out, idx] = parseext(len, bytes, idx)
    ...
    switch exttype
        case 255
            switch len
                case 4
                    seconds = bytes2scalar(bytes(idx+1:idx+4), 'uint32');
                    out = datetime(seconds,'ConvertFrom','posixtime');
                case 8
                    freak = bytes2scalar(bytes(idx+1:idx+8), 'uint64');
                    nanoseconds = bitshift(freak, -34);
                    seconds = bitand(uint64(17179869183),freak,'uint64');
                    out = datetime(seconds,'ConvertFrom','posixtime') + milliseconds(nanoseconds/1000);
                case 12
                    nanoseconds = bytes2scalar(bytes(idx+1:idx+4), 'uint32');
                    seconds = bytes2scalar(bytes(idx+5:idx+12), 'uint64');
                    out = datetime(seconds,'ConvertFrom','posixtime') + milliseconds(nanoseconds/1000);                    
            end
        otherwise            
            out.type = bytes(idx);
